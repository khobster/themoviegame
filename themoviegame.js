/* themoviegame.js – v2.2 (fix typing, 2-min sync) */
...
function guess(){
  ...
  if(g.toLowerCase()===current.answer.toLowerCase()){
    ...
    RES.textContent='Nope'; SOUND_BAD.play(); setLS('streak',0);
  }
  updateHUD();
}
...
function hint(){
  ...
  setLS('score',Math.max(0,get('score')-1)); updateHUD(); toast('-1 point');
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  BTN.onclick=guess;
  /* fix: allow typing, only intercept Enter */
